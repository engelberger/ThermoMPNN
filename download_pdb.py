...
def download_pdb(pdbcode, datadir, downloadurl="https://files.rcsb.org/download/", assembly=-1):
    ...
    if assembly == -1:
        ...
        suffix = f".pdb{assembly}" 
    pdbfn = pdbcode + suffix
    print(pdbfn)
...
def main():
    ...
    data_dir = '../pdb_files/homologue_free'
    if not os.path.isdir(data_dir):
        os.makedirs(data_dir)
    # load pdb codes
    df = pd.read_csv('../data_all/testing/fireprot_HF.csv', header=0, usecols=['pdb_id_corrected', 'uniprot_id'], dtype='str')
    ...
    for code in codes:
        ...
        for assembly in range(-1, 10):
        	download_pdb(code, data_dir, assembly=-1)
main()
