...
def get_protein_mpnn(cfg, version="v_48_020.pt"):
    ...
    if hasattr(cfg.model, "model_weights_path"):
        checkpoint_path = os.path.abspath(
            os.path.join(os.getcwd(), cfg.model.model_weights_path)
        )
    else:
        checkpoint_path = os.path.abspath(
            os.path.join(os.getcwd(), "vanilla_model_weights", version)
        )
...
class TransferModelPH(nn.Module):
    """A variant of TransferModel that predicts pH instead of ddG."""
    ...
    def __init__(self, cfg):
        ...
        # Modified to output a single pH value instead of per-amino acid predictions
        hid_sizes = [HIDDEN_DIM * self.num_final_layers + EMBED_DIM]
        hid_sizes += self.hidden_dims
        hid_sizes += [1]  # Single output for pH prediction
        ...
        self.ph_out = nn.Sequential()

        for sz1, sz2 in zip(hid_sizes, hid_sizes[1:]):
            self.ph_out.append(nn.ReLU())
            self.ph_out.append(nn.Linear(sz1, sz2))
    ...
    def forward(self, pdb, mutations, tied_feat=True):
        ...
        for mut in mutations:
            ...
            if self.num_final_layers > 0:
                hid = mpnn_hid[0][
                    mut.position
                ]  # MPNN hidden embeddings at mut position
                inputs.append(hid)
            ...
            # passing vector through lightattn if enabled
            if self.lightattn:
                lin_input = torch.unsqueeze(torch.unsqueeze(lin_input, -1), 0)
                lin_input = self.light_attention(lin_input, mask)

            # Direct pH prediction without amino acid-specific outputs
            ph_pred = self.ph_out(lin_input)
